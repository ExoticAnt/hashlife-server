...
using Implementation = simple_implementation::Simple_Implementation;
using HLException = simple_implementation::HLException;

BOOST_AUTO_TEST_CASE(constructors_test)
{

	//auto universe = Implementation(cols, rows);
	//checks for minimum dimensions
	BOOST_CHECK_THROW(Implementation(1,1), HLException);
	BOOST_CHECK_THROW(Implementation(5, 1), HLException);
	BOOST_CHECK_THROW(Implementation(0, 10), HLException);

	//invalid access:
	auto universe = Implementation(4, 4);
	BOOST_CHECK_THROW(universe.set(5, 0, 1), HLException);
	BOOST_CHECK_THROW(universe.set(0, 5, 1), HLException);

	//simple set and to string
	universe = Implementation(4, 4);
	universe.set(2, 3, 1);
...
BOOST_AUTO_TEST_CASE(life_test)
{
	...
	//also is in a corner
	auto universe = Implementation(5, 5);
	universe.set(3, 2, 1);
	universe.set(3, 3, 1);
	universe.set(3, 4, 1);
	...
	BOOST_TEST(universe.to_s() == "0000000\n0000000\n0001000\n0001000\n0001000\n");

	//a simple dot dissapears after 1 step.
	universe = Implementation(5, 5);
	universe.set(2, 2, 1);
